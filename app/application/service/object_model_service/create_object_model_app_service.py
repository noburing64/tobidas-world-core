...
class CreateObjectModelAppService(AbsCreateObjectModelAppService):
    ...
    def handle(self, param: CreateObjectModelParam):
        ...
        # PIDを登録する
        self.__object_model_repository.UpdatePid(object_model_id, pid)
        # MVGの実行
        ...
        if not os.path.exists(f"./storage/{dir_name}/output/reconstruction/model_data.obj"):
            return {"msg": "error"}
        ...
        # ファイルの保存（アップロード）
        local_file_path = f"./storage/{dir_name}/output/reconstruction"
        file_path = f"{dir_name}/obj"
        file_size = self.__object_model_service.store(local_file_path, file_path)
        self.__object_model_service.delete_local_files(local_file_path)
